#!/usr/bin/env python3
"""Generate Hub model catalog payloads with the shared engine.
...
CONFIGURATION = engine.GeneratorConfiguration(
    component_name="Hub model catalog",
    kustomize_path=Path("applications/hub/helm-catalog/kustomize"),
    output_path=Path("applications/hub/helm-catalog/manifests"),
    generator_script="scripts/generate-hub-catalog-helm-manifests.py",
    synchronize_script="scripts/synchronize-hub-manifests.sh",
    crds_payload_filename=None,
    hand_written_resources=(
        ("Secret", "model-catalog-hf-api-key", False),
        ("Secret", "model-catalog-postgres", False),
    ),
    extracted_documents=(
        (
            "Secret",
...
def main():
    return engine.command_line(
        CONFIGURATION,
        description="Generate payloads for the Kubeflow Hub model catalog Helm chart.",

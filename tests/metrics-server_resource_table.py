...
COMPONENT_RULES = {
    ...
    "Notebook Controller": {"keywords": ["notebook-controller"]},
    "PVC Viewer Controller": {"keywords": ["pvcviewer"]},
    "Tensorboard Controller": {"keywords": ["tensorboard"]},
    "Central Dashboard": {"keywords": ["kubeflow-dashboard"]},
    "Profiles + KFAM": {"keywords": ["profiles", "profile-controller"]},
    "PodDefaults Webhook": {"keywords": ["poddefaults-webhook", "poddefaults"]},
    "Jupyter Web Application": {"keywords": ["jupyter-web-app", "jupyter"]},
    ...
    "KServe": {
        ...
        "exclude": ["models-web-application", "kserve-ui"],
    },
    "KServe UI": {"keywords": ["models-web-application", "kserve-ui"]},
    "Kubeflow Pipelines": {
        "keywords": [
            "ml-pipeline",
            "workflow-controller",
    ...
    "Cert Manager": {"namespaces": ["cert-manager"], "keywords": ["cert-manager"]},
    "Dex": {"namespaces": ["auth"], "keywords": ["dex"]},
    "OAuth2-Proxy": {"namespaces": ["oauth2-proxy"], "keywords": ["oauth2-proxy"]},
}
...
def generate_table(
    component_resources, actual_usage, manifest_requests, live_storage=None
):
    """Generate markdown table from component resources"""
    print("## Resource Usage by Components")
    print()
    print(
        "The following table shows the resource requirements for each Kubeflow components:"
    )
    print()
    ...
    for component in COMPONENT_ORDER:
        if component in component_resources:
            ...
            print(
                f"| {component} | {resources['cpu']}m | {resources['memory']}Mi | {live_stor}GB |"
            )

    print(
        f"| **Total** | **{totals['cpu']}m** | **{totals['memory']}Mi** | **{totals['live_storage']}GB** |"
    )
    print()
